...
public:
    ...
    static Packet move(Id id, rb3202::Angle pos, std::chrono::milliseconds t) {
    ...
    static Packet move(Id id, rb3202::Angle pos) {
        float position = pos.deg();
        if (position < 0 || position > 240)
            ESP_LOGE("LX16A", "Position out of range");
    ...
    static Packet limit(Id id, rb3202::Angle b, rb3202::Angle t) {
        int bottom = b.deg();
        int top = t.deg();
